...
TRACK1_USAGE_DECISIONS: dict[str, UsageDecision] = {
    ...
    "A-DS13": _decision(
        ...
        proposal_pages=(7, 8, 9),
        status="EVIDENCE_REQUIRED",
        decision="후보 장소가 존재하지만 TourAPI contentId·API 응답 스냅샷이 없어 A-DS13 직접 사용을 증명할 수 없습니다.",
        sheet_tabs=("후보 장소", "로컬 설명"),
        code_consumers=("backend.app.courses.data.COURSE_DETAILS[].itinerary",),
        api_fields=("CourseDetailResponse.itinerary", "CourseDetailResponse.localPoints"),
        ui_surfaces=("코스 상세 > 코스 순서", "코스 상세 > 로컬 포인트"),
        next_evidence=("TourAPI contentId", "사용 API 필드", "응답 스냅샷 해시"),
    ),
